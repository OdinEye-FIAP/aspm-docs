...
Isso tem duas lacunas, uma de confiabilidade e uma de escopo:
...
sequenceDiagram
    ...
    alt payload utilizável (repo individual)
        ...
        CH->>CH: background_tasks: auto-scaffold (inalterado)
...
| `controller/ping_controller.py` | Troca `await publisher.publish(topic=settings.topic_repository_registered, ...)` por `await get_pequod_client().register_repository(registration_event)`. Falha propaga (mesmo comportamento atual — webhook retorna erro, GitHub reentrega). |
...
2. **captain-hook**: troca publish por REST. A partir daqui, os tópicos `repository.registered.v1`/`.unregistered.v1` deixam de receber mensagens.
...
Fase B pode ir no mesmo deploy do captain-hook do passo 2, ou em um deploy seguinte — não tem dependência com a Fase A além de reaproveitar o `PequodClient`/registro já migrado.
...
- `docs/captain-hook.md`: os dois diagramas sequenciais de `ping`/`installation` (adicionados no PR #19) passam a refletir REST + baseline, não mais só Kafka de registro.
- `docs/reference/kafka-topics.md`: remove `repository.registered.v1`/`.unregistered.v1` e as DLQs correspondentes.
- `docs/reference/http-endpoints.md`: adiciona `/internal/repositories/register`/`unregister` no pequod; atualiza a lista de "Side effects" do `POST /webhook` do captain-hook.
