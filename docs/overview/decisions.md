...
- **Confiabilidade do producer em `installation`/`installation_repositories` → `repository.registered.v1`/`.unregistered.v1` (levantado por Leandro em 2026-09-10 — EM PLANEJAMENTO):** o lado consumer no pequod é sólido — `RepositoryRegistrationConsumer` verifica assinatura HMAC, só comita offset depois do handler rodar, tem retry com backoff (`call_with_retry`) e manda pra DLQ (`repository.registration.pequod.dlq`) se esgotar as tentativas; o handler faz upsert idempotente + audit log na mesma transação. O lado producer no captain-hook não tem o mesmo rigor: `ping` propaga falha de publish pra resposta HTTP do webhook (o GitHub reentrega); mas `installation`/`installation_repositories` roda em `background_tasks` — depois do `200 OK` já devolvido ao GitHub — e `_publish_registrations_sequentially`/`_publish_unregistrations_sequentially` (`controller/installation_controller.py`) capturam a exceção por repositório, logam (`logger.exception`) e seguem pro próximo. Se o Kafka cair nesse momento, o lote inteiro daquele evento nunca chega a ser publicado: não há DLQ possível (a mensagem nunca saiu do captain-hook), não há retry, e o GitHub não reentrega porque o webhook, do lado dele, já teve sucesso. **Direção acordada em 2026-09-10:** simplificar removendo o Kafka desse fluxo inteiro — captain-hook passa a chamar o pequod diretamente por REST síncrono (mesmo padrão do §16), eliminando `repository.registered.v1`/`.unregistered.v1` e as DLQs correspondentes. Confirmado que são os únicos producer/consumer desses tópicos. **Atualização 2026-09-11:** confirmado também que `ping`/`installation`/`installation_repositories` continuam precisando de Kafka pro lado captain-hook→moby-dick (disparo de Security Baseline no `main` do repo recém-instalado) — são dois fluxos independentes. **Proposta de implementação completa (Fase A: registro via REST; Fase B: Security Baseline em ping/install) em [`design/ping-install-rest-migration.md`](../design/ping-install-rest-migration.md) — PR aberto para revisão.**
- **Ideia para o radar (Leandro, 2026-09-11): tópicos Kafka por scanner** (`jobs.orchestration.sonar`, `.semgrep`, `.trivy`, `.zap` em vez de um `jobs.orchestration` único). Motivação citada: processar findings sem esperar todos os scanners terminarem, repensar o uso do Quality Gate, manutenção mais fácil dos fluxos. Pré-requisito já satisfeito hoje: os builders de cada scanner (`build_job`/`build_baseline_job`) já são agnósticos ao trigger (PR, push, e futuramente ping/install) — não precisa de mudança pra isso funcionar por trás de tópicos separados. **Ponto a verificar antes de desenhar:** a ingestão de finding via `findings.raw` já não espera todos os scanners hoje — pequod processa e persiste cada SARIF conforme chega, scanner a scanner (`controller/ingest_controller.py`); só a **decisão do Quality Gate** (não o processamento do finding) espera todos os `expected_scanners` completarem, por design (é um gate). Vale confirmar exatamente qual ganho os tópicos separados trariam que o comportamento atual já não entrega, antes de multiplicar tópicos/DLQs/consumer groups (hoje 1 tópico + 1 DLQ + 1 consumer group em moby-dick; por-scanner viraria 4 de cada, competindo com o princípio de moby-dick ser agnóstico a scanner — §11).
- **Ideia para o radar (Leandro, 2026-09-11): repensar `/repos/{owner}/{repo}/live-info`** (hoje usado pelo heimdall-dashboard pra issues + dependências). Já busca ao vivo na API do GitHub, sem cache e sem Kafka — mas está lento na prática. Antes de redesenhar achando que "usar a API do GitHub deixa sempre atualizado", vale investigar a causa real da lentidão atual (chamadas sequenciais? paginação? rate limit da API do GitHub?), já que o endpoint já é live hoje — trocar por API do GitHub não é o que ele já não faça. Ideia relacionada: listar PRs do repositório (não existe hoje, só issues + dependências) — seria endpoint novo/campo novo no mesmo estilo, sem Kafka nem moby-dick envolvidos.
- **Documentação futura desejada (Leandro, 2026-09-11): diagrama fim-a-fim do processo de scan até o risco consolidado.** Hoje os diagramas existentes cobrem trechos (fluxo de PR em `architecture.md`, check_run em `check-run.md`, ping/installation em `captain-hook.md`), mas nenhum mostra a jornada completa desde o scanner terminando (`findings.raw`) até a `Finding` ser persistida, passar por candidate clustering, clustering semântico do TARS e chegar a um `consolidated_risk` exibido no heimdall-dashboard. Ainda não desenhado — só registrado o pedido.
